...
pub fn validate_plate(plate: &str) -> bool {
    ...
    if char_count < 8 || char_count > 9 {
        tracing::warn!("Plate length invalid: {} chars (expected 8-9) for '{}'", char_count, normalized);
        return false;
    }
    ...
    tracing::info!("Validating plate '{}', length: {}, chars: {:?}", normalized, chars.len(), chars);
    ...
    let is_letter = |c: &char| -> bool {
        ...
        if !is_cyrillic && !is_latin {
            tracing::warn!("Character '{}' (U+{:04X}) is not a letter (cyrillic: {}, latin: {})", 
                c, code, is_cyrillic, is_latin);
        }
    ...
    if !is_letter(&chars[0]) {
        tracing::warn!("First character '{}' (U+{:04X}) is not a letter for plate '{}'", 
            chars[0], chars[0] as u32, normalized);
        return false;
    }
    ...
    if !chars[1..4].iter().all(|c| c.is_ascii_digit()) {
        tracing::warn!("Characters 1-3 are not all digits: {:?} for plate '{}'", &chars[1..4], normalized);
        return false;
    }
    ...
    for (i, c) in chars[4..6].iter().enumerate() {
        if !is_letter(c) {
            tracing::warn!("Character {} '{}' (U+{:04X}) at position {} is not a letter for plate '{}'", 
                i + 4, c, *c as u32, i + 4, normalized);
            return false;
        }
    }
    ...
    if !chars[6..].iter().all(|c| c.is_ascii_digit()) {
        tracing::warn!("Last characters are not all digits: {:?} for plate '{}'", &chars[6..], normalized);
        return false;
    }
