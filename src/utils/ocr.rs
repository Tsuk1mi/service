...
pub async fn recognize_plate_from_image(image_data: &[u8]) -> AppResult<String> {
    ...
    Err(AppError::Internal(
        "OCR not configured. Please set OCR_API_URL environment variable or enter plate manually".to_string()
    ))
}
...
async fn recognize_via_api(api_url: &str, image_data: &[u8]) -> AppResult<String> {
    ...
    if !response.status().is_success() {
        return Err(AppError::Internal(format!("OCR API returned error: {}", response.status())));
    }
